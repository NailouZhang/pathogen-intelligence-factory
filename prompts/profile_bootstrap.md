...
默认：score >= 6 为 accept；3–5 为 review；<3 为 reject。候选阶段使用数据库返回的标题、摘要和元数据完成 Python 全量复核；不在 Top-N 选择之前抓取全文或新闻正文。明确相关和边界记录进入去重与排序，Python 无法可靠定义的边界记录交给 Gemini/Groq 紧凑复核。
...
- 初始检索只使用人工 `search_strategy.concepts` 中最多 5 个彼此有区分度的核心概念。模型不得增加、删除、改写或重排这些概念。
- 这 5 个概念应覆盖常见身份、重点成员/型别、疫情热点、临床或干预、基因组或监测等不同信息面；不得仅堆叠同义全称和缩写。
- 各数据库由 Python 编译为其原生查询形式：PubMed 使用简短概念并利用 Automatic Term Mapping；Europe PMC 使用单一自然语言概念并利用自由文本扩展；Semantic Scholar、OpenAlex、Crossref 使用短文本查询；新闻源使用自然语言短语。
- 完整身份词、历史名、成员白名单、限定缩写、蛋白、宿主、疾病、排除词等富词库不再扩增初始查询次数，而是用于 100% 候选的 Python 相关性复核。
- Python 对全部候选提取身份命中、频次、限定缩写、上下文、排除实体、来源收敛和检索概念；明确相关或明确无关者直接判定。
- 只有 Python 无法可靠判定的边界记录进入 Gemini/Groq 紧凑证据复核；按 Token 预算动态分批直到不确定队列为空，不按固定篇数截断。
- DOI、PMID、规范化标题、作者、年份和 URL 用于确定性去重；新闻同时执行相似标题和多来源合并。
- 先按相关性、来源收敛、研究设计、热点信号、时效和机构权威性排序并选择最多 50 篇文献与 50 条新闻。
- 只有已选中的最终展示集合才访问合法开放获取全文、出版者公开页面或新闻正文；未入选候选不得进行昂贵内容补全。
- 深度翻译、五要素和综合综述只处理最终展示集合。
