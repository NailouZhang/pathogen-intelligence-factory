...
from .utils import dump_json, html_escape, sha256_text
...
<section style="margin:0 0 28px;border:1px solid {COLORS['line']};border-radius:10px;overflow:hidden;background:#fff;">
  ...
  <section style="padding:18px;">
    <p style="margin:0 0 5px;color:{COLORS['paper_green']};font-size:12px;font-weight:bold;">学术文献 · {'综述' if kind == 'review' else '研究'} · {html_escape(work.get('evidence_level') or 'E0')}</p>
...
<article class="card paper">
  ...
  <div class="card-body">
    <div style="font-size:12px;color:{COLORS['paper_green']};font-weight:700;margin-bottom:7px;">学术文献 · {'综述' if kind == 'review' else '研究'} · {html_escape(work.get('evidence_level') or 'E0')}</div>
...
<section style="margin:0 0 20px;border:1px solid #fed7d7;border-radius:9px;overflow:hidden;background:#fff;">
  ...
  <section style="padding:17px;">
    <h3 style="margin:0;color:#9b2c2c;font-size:18px;line-height:1.45;">{html_escape(title_zh)}</h3>
...
<article class="card news">
  ...
  <div class="card-body">
    <div class="lang-zh"><h3 style="color:#9b2c2c">{html_escape(title_zh)}</h3><div class="title-en">{html_escape(article.get('title'))}</div><div class="translated-body"><strong>新闻正文/简要中文翻译</strong>{html_escape(translated)}</div></div>
...
<section class="overview"><h2>💡 今日核心综述 / Summary</h2><p class="lang-zh">{html_escape(overview.get('zh'))}</p><p class="lang-en" hidden>{html_escape(overview.get('en'))}</p></section>
