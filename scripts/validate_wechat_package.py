...
def main() -> int:
    ...
    if budget_path.is_file():
        ...
        if budget.get('policy_version') in {'v16.1-wechat-visible-text-budget-2', 'v17-wechat-visible-text-budget-audit-only-1'}:
            ...
            if budget.get('operational_notice_rendered') is True:
                raise SystemExit('operational WeChat budget notice must remain audit-only')
            if '微信公众号篇幅说明' in content_text:
                raise SystemExit('operational WeChat budget notice leaked into article.html')
